...
controller.hears(['(.*)license(.*)Office(.*)', '(.*)Office(.*)license(.*)', '(.*)key(.*)Office(.*)', '(.*)Office(.*)key(.*)'],
    'direct_message,direct_mention', function(bot, message) {

        bot.reply(message,
            ':key: PGD67-JN23K-JGVWW-KTHP4-GXR9G');
        bot.reply(message,
            'Tell Clippy I said Hi!');

    });


controller.hears(['(.*)api(.*)facebook(.*)', '(.*)facebook(.*)api(.*)', '(.*)key(.*)facebook(.*)', '(.*)facebook(.*)key(.*)'],
    'direct_message,direct_mention', function(bot, message) {

        bot.reply(message,
            ':key: 1169157474746195');
        bot.reply(message,
            'Zuck is gonna love your new Facebook app!');
...
// API response
controller.hears(['API','list'],'direct_mention,direct_message',function(bot,message) {
    ...
    controller.storage.users.get(message.user, function(err, user) {
        ...
        var reply = {
            text: 'Here are your tokens. Say add item to add another.',
            attachments: [],
        }
